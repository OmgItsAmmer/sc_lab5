...
public void testWrittenByNoMatches() {
    ...
    assertTrue("expected empty list when no tweets match author", writtenBy.isEmpty());
}

@Test
public void testWrittenByCaseSensitivity() {
    List<Tweet> writtenBy = Filter.writtenBy(Arrays.asList(tweet1, tweet2), "ALYSSA");
    // Assuming username match is case-sensitive, expect no results
...
public void testInTimespanEmptyResult() {
    ...
    Instant end = Instant.parse("2015-01-01T01:00:00Z");
...
public void testInTimespanExactBoundary() {
    Instant start = d1; // tweet1 timestamp
    Instant end = d1;   // same as start
    List<Tweet> inTimespan = Filter.inTimespan(Arrays.asList(tweet1, tweet2), new Timespan(start, end));
    assertEquals("expected only tweet1 at exact boundary", 1, inTimespan.size());
...
public void testContainingNoMatches() {
    List<Tweet> containing = Filter.containing(Arrays.asList(tweet1, tweet2), Arrays.asList("nonexistentword"));
    assertTrue("expected empty list when no tweets contain any word", containing.isEmpty());
}
...
public void testContainingCaseInsensitive() {
    ...
    assertEquals("expected both tweets containing 'rivest' ignoring case", 2, containing.size());
